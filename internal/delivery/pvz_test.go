...
func TestGetPVZHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "invalid limit",
			role: "moderator",
			queryParams: fmt.Sprintf("startDate=%s&endDate=%s&page=1&limit=bad",
				startDate.Format(time.RFC3339),
				endDate.Format(time.RFC3339),
			),
			requestBody:  nil,
		...
		},
		{
			name: "invalid page",
			role: "moderator",
			queryParams: fmt.Sprintf("startDate=%s&endDate=%s&page=bad&limit=10",
				startDate.Format(time.RFC3339),
				endDate.Format(time.RFC3339),
			),
			requestBody:  nil,
